...
impl Node {
    ...
    fn new(node_id: NodeId, env: &SimEnv) -> Self {
        Self {
            node_id,
            ...
            last_frame_mem: 0.0,
            instance_cache_policy: RefCell::new(
                env.help()
                    .config()
                    .mech
                    .new_instance_cache_policy(
                        // node_id, 
                        // env.into()
                    ),
            ),
        ...
        for &(req_id, fnid) in self.pending_tasks.borrow_mut().iter() {
            ...
            if let Some(mut fncon) = self.container_mut(fnid) {
                ...
                self.instance_cache_policy
                    .borrow_mut()
                    .get(fnid, fncon.borrow().last_frame_mem)
                    .unwrap();
                // add to container
