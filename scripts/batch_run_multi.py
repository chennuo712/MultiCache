...
params=['request_freq','dag_type','no_mech_latency','consistency_level']
        ...
        def compose_mech_other():
            def one_composition():
                ...
                for i in range(needrun):
                    # 设置多级缓存配置
                    env.config['cache_config'] = {
                        'l1_ratio': 0.60,
                        'l2_ratio': 0.30,
                        'l3_ratio': 0.10,
                        'l1_capacity': 100
                    }
                    # 从 env.config['consistency_level'] 获取一致性级别
                    c_level = env.config.get('consistency_level', 'eventual')
                    env.config['consistency'] = {
                        'level': c_level,
                        'update_interval': 10,
                        'max_inconsistency_window': 5,
                        'propagation_delay': 1,
                        'enabled': True if c_level != 'none' else False
                    }
                    env.reset()
                    env.step(1)
